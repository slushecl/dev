...
Example:
  ...
  if {$patCount < 1} {
    return -code error \
    "No file types listed. Please indicate a file type to load.\n$usage"
  }
  if {$patCount > 2} {
...
proc fragcolor {monomerid args} {
  foreach arg $args {
    if {$arg>207 || $arg<0} {
      puts "$arg out of range. Value must be between 1 and 207 inclusive."
      continue
    } else {
      set selalpha [atomselect $monomerid "chain A and fragment $arg"]
      set selbeta [atomselect $monomerid "chain B and fragment $arg"]
      if {[$selalpha num]>0} {
        mol selection "fragment $arg"
        mol representation Lines 3.0
        mol material Opaque
        mol color ColorID 1
        mol addrep $monomerid
      }
      if {[$selbeta num]>0} {
        mol selection "fragment $arg"
        mol representation Lines 3.0
